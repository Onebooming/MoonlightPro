 ...
 * @LastEditTime: 2025-02-17 22:43:28
 * @FilePath: /MoonLightPro/src/server/serverdome1.cpp
 * @Description: server demo   
 */
#include "../common/include/mnserversocket.h"
...
void clientHandler(int client_sock) {
    ...
    if (bytes_received > 0) {
        ...
        std::vector<uint8_t> &data = *reinterpret_cast<std::vector<uint8_t> *>(buffer);
        auto rMes = MoonLight::Message::deserialize(data);
        std::cout << "content = " << std::string(rMes->getDataContent()) << std::endl;
        std::string response = "Message received from client is : " + std::string(rMes->getDataContent());
